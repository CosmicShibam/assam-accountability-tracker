...
const START_DATE = new Date("2026-05-20T00:00:00+05:30");
const END_DATE = new Date("2031-05-20T00:00:00+05:30");

function getTimeLeft(end: Date, now: Date) {
...
export default function HeroSection() {
  ...
  const timeLeft = getTimeLeft(END_DATE, now);
  const daysInOffice = Math.max(0, Math.floor((now.getTime() - START_DATE.getTime()) / (1000 * 60 * 60 * 24)));
  const daysRemaining = Math.max(0, Math.floor((END_DATE.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)));
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-8 py-12 md:py-20 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-8 items-center">
          ...
          <div className="lg:col-span-5 flex flex-col gap-6 text-center lg:text-left z-10">
            ...
            >
              <h1 className="font-heading leading-tight flex flex-col">
                <span className="text-2xl md:text-3xl tracking-[0.1em] text-primary mb-2 font-medium uppercase">Assam</span>
                <span className="text-5xl md:text-6xl lg:text-7xl font-bold tracking-tight mb-2 text-on-surface drop-shadow-sm">Promise</span>
                <span className="text-4xl md:text-5xl font-light italic text-on-surface-variant">Tracker</span>
              </h1>
            </motion.div>
            ...
            <motion.div
              ...
              className="w-24 h-1.5 rounded-full bg-primary/80 mx-auto lg:mx-0 my-1 shadow-[inset_1px_1px_2px_rgba(255,255,255,0.4)]"
            />
            ...
            <motion.p
              ...
              className="text-base md:text-lg text-on-surface-variant leading-relaxed max-w-xl mx-auto lg:mx-0 font-medium"
            >
              "Assam's public ledger tracking promises, policies, projects, and commitments made by the Government of Assam during the 2026–2031 term."
            </motion.p>
          </div>

          {/* Right Column: Gov Info & Countdown */}
          <div className="lg:col-span-4 flex flex-col gap-8 z-10">
            ...
            >
              {[
                { label: "Chief Minister", value: "Himanta Biswa Sarma" },
                { label: "Government", value: "BJP-led NDA" },
                { label: "Term", value: "2026–2031" },
                { label: "Capital", value: "Dispur, Assam" },
                { label: "Days In Office", value: daysInOffice.toLocaleString() },
                { label: "Days Remaining", value: daysRemaining.toLocaleString() },
              ].map((info, idx) => (
                <div key={idx} className="clay-recessed p-4 flex flex-col justify-center transition-all hover:bg-surface-variant/30 rounded-2xl">
                  <span className="text-[9px] sm:text-[10px] uppercase tracking-widest text-outline font-bold mb-1.5">{info.label}</span>
                  <span className="font-heading font-semibold text-on-surface text-sm sm:text-base leading-tight">{info.value}</span>
                </div>
              ))}
            </motion.div>
            ...
            <motion.div 
              ...
              className="flex flex-col pt-6 border-t border-outline-variant/30"
            >
              <h3 className="text-[10px] md:text-xs uppercase tracking-[0.2em] text-primary font-bold mb-5 text-center lg:text-left">
                Time Remaining In Current Term
              </h3>
              ...
              <div className="grid grid-cols-6 gap-2 sm:gap-3 text-center">
                {[
                  ...
                  <div key={idx} className="flex flex-col items-center">
                    <span className="font-heading font-bold text-xl sm:text-2xl text-on-surface tabular-nums leading-none mb-2 clay-card w-full aspect-square flex items-center justify-center rounded-xl bg-surface">
                      {String(unit.value).padStart(2, '0')}
                    </span>
                    <span className="text-[8px] sm:text-[9px] font-bold uppercase tracking-wider text-outline">
                      {unit.label}
                    </span>
                  </div>
                ))}
