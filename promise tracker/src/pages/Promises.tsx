...
export default function Promises() {
  ...
  const filteredPromises = useMemo(() => {
    const filtered = promises.filter(p => {
      ...
      const matchStatus = statusFilter === "All" || getNormalizedStatus(p.status) === statusFilter;
    ...
    return filtered;
  }, [promises, searchQuery, categoryFilter, statusFilter]);
          ...
          <div className="flex flex-wrap gap-3 items-center">
            ...
            {statusFilter !== "All" && (
              <button 
                ...
                className="clay-chip text-xs px-4 py-2 rounded-full font-medium transition-colors flex items-center gap-1 bg-secondary-container text-on-secondary-container font-bold"
                title="Clear Status Filter"
              >
                Status: {statusFilter.replace('_', ' ')}
                <span className="ml-1 text-on-secondary-container">&times;</span>
              </button>
            )}
